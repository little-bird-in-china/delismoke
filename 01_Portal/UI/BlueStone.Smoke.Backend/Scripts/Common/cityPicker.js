...
;(function($){
	...
	function otherBlur() {
	    var $this = $(this);
	    var $input = $this.offsetParent().find(".form-row input");
	    $input.blur();
	}
	...
	//			distInfo=searchCityData(cityInfo.children,option.dist);
				...
				for(key in provInfo.city){
				    cityList += '<div anum="' + cityData[0].city[0].area[key].sysno + '" class="picker-item">' + provInfo.city[key].name + '</div>';
				}
				for(key in cityInfo.area){
				    distList += '<div anum="' + cityData[0].city[0].area[key].sysno + '" class="picker-item">' + cityInfo.area[key].name + '</div>';
				}
	...
	CityPicker.prototype.init=function(data){
		...
		for(key in cityData){
		    provList += '<div anum="' + cityData[key].sysno + '" class="picker-item">' + cityData[key].name + '</div>';
		}
		for(key in cityData[0].city){
		    cityList += '<div anum="' + cityData[0].city[key].sysno + '" class="picker-item">' + cityData[0].city[key].name + '</div>';
		}
		for (key in cityData[0].city[0].area) {
		    distList += '<div anum="' + cityData[0].city[0].area[key].sysno + '" class="picker-item">' + cityData[0].city[0].area[key].name + '</div>';
		}
			...
			provGroup.myPicker({
				...
				callback:function(obj,index){
					...
					for(key in city.city){
					    cityList += '<div anum="' + city.city[key].sysno + '" class="picker-item">' + city.city[key].name + '</div>';
					}
					for(key in city.city[0].area){
					    distList += '<div anum="' + city.city[0].area[key].sysno + '" class="picker-item">' + city.city[0].area[key].name + '</div>';
					}
					cityGroup.html(cityList).css({
						"-webkit-transform":"translate3d(0px,80px,0px)",
						"-webkit-transition-duration":"0s"
			...
			cityGroup.myPicker({
				...
				callback:function(obj,index){
					...
					for(key in dist.area){
					    distList += '<div anum="' + dist.area[key].sysno + '" class="picker-item">' + dist.area[key].name + '</div>';
					}
					distGroup.html(distList).css({
						"-webkit-transform":"translate3d(0px,80px,0px)",
						"-webkit-transition-duration":"0s"
			...
			cancelBtn.tap(function(e){
				...
				if(option.cancel){
					option.cancel({
					    ...
					    distSysno: distGroup.find(".active").attr("anum")
					});
				}
				closePicker();
			});
	...
	$.fn.cityPicker=function(options){
		...
		this.each(function(){
		    ...
		    $.getJSON(QueryAreaUrl, function (data) {
